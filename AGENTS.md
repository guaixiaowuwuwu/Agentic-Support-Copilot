...
- 当前身份模型：本地/demo 可用 header 身份，staging/production 使用 trusted identity headers；真实用户场景仍需接入企业 SSO / OIDC / JWT / API gateway。
...
- header-based 本地身份只适合本地/demo；staging/production 必须使用不可由浏览器伪造的可信身份上下文。
...
- `NEXT_PUBLIC_SUPPORT_COPILOT_*` 身份配置仅用于本地/demo；真实用户场景需要 SSO / OIDC / JWT 或可信网关注入。
- 前端已按 `support_agent`、`approver`、`knowledge_admin`、`admin` 生成导航和默认工作台，但真实企业试用仍需接入 SSO 角色映射。
- 已有 `/knowledge` 知识库管理页面，支持文档新增和 embedding ingestion。
- 已有审计日志查询 API 和 `/audit` 页面；审计记录仍需进一步筛选、检索和导出能力。
